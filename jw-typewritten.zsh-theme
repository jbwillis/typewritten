...
declare -A archs
archs=(
    ["arm64"]="%F{$tw_colors[virtual_env]}m1"
    ["x86_64"]="%F{$tw_colors[virtual_env]}x86"
)
arch=$(uname -m)
arch_label=$archs[$arch]
...
tw_redraw() {
  ...
  if [ "$tw_layout" = "half_pure" ]; then
    ...
    if [ "$tw_layout" = "pure" ]; then
      PROMPT="$BREAK_LINE$arch_label $tw_displayed_wd$tw_git_arrow_info$BREAK_LINE$tw_env_prompt"
      RPROMPT=""
    fi;
